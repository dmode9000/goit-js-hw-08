...
const galeryMarkup2 = images.reduce((markup, { preview, original, description }) => {
  return (markup += `
...
const createTag = (tagName, attributesObj) => {
  ...
  Object.entries(attributesObj).forEach(([attrName, attrValue]) => tag.setAttribute(attrName, attrValue));
  return tag;
};
...
galeryContainer.addEventListener("click", (event) => {
  ...
  if (event.target !== event.currentTarget) {
    const modal = basicLightbox.create(`
      	<img id="modal" src="${event.target.closest(".gallery-image").dataset.source}">
      `);
    modal.show();
  }
});
